...
def _err(e: Exception) -> dict:
    ...
    if _is_production_mode():
        ...
        if os.environ.get("DDB_DEBUG_ERRORS"):
            print(
                f"[ddb-security] Original error: code={code}, message={message}",
                file=sys.stderr,
                flush=True,
            )
    ...
    for char in _DANGEROUS_CHARS:
        if char in expression:
            return {
                "error": True,
                "code": "ValidationException",
                "message": f"Expression contains prohibited character: {char!r}",
            }
...
def _validate_expression_attributes(
    names: dict[str, str] | None = None, values: dict[str, Any] | None = None
) -> dict | None:
...
def query(
    ...
    attr_err = _validate_expression_attributes(
